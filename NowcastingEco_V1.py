...
class NowcastingEco:
    ...
    def _theme_filtering(self):   
        ...
        if theme == 'CONSUMPTION':
            ...
            df = df[(df['cleaned_themes'].apply(lambda x: any(keyword in x for keyword in theme_filter))) | (self.df['cleaned_xml'].apply(lambda x: any(keyword in x for keyword in theme_filter)))]
            ...
            df = df[(df['cleaned_themes'].apply(lambda x: any(keyword in x for keyword in theme_filter))) | (self.df['cleaned_xml'].apply(lambda x: any(keyword in x for keyword in theme_filter)))] #and self.df['cleaned_url'].apply(lambda x: any(keyword in x for keyword in filter))]

        elif theme == 'EMPLOYMENT':
            theme_filter = ['EMPLOYMENT','UNEMPLOYMENT', 'LAYOFFS', 'SALARY', 'LABOR', 'STRIKES', 'UNIONS', 'WORKERS']
            df = df[(df['cleaned_themes'].apply(lambda x: any(keyword in x for keyword in theme_filter))) | (self.df['cleaned_xml'].apply(lambda x: any(keyword in x for keyword in theme_filter)))] #and self.df['cleaned_url'].apply(lambda x: any(keyword in x for keyword in theme_filter))]
    ...
    def read_country_data(self):
        ...
        if self.country == 'Egypt':
            path = '/content/Bloomberg_Data_Egypt.xlsx'
